...
def plot(image):
    # sample_target = target[0] + 1j * target[1]
    with torch.no_grad():
        # show_coils(np.log(np.abs(torch.Tensor.cpu(sample_target).numpy()) + 1e-9))
        show_coils(np.abs(torch.Tensor.cpu(image[0]).numpy()))
...
class UnetModule(MriModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.calculate_loss(output, target, torch.Tensor([12]).cuda())

        self.log("loss", loss.detach())

        return loss

    def calculate_loss(self, output, target, max_value):
        if self.loss == F.l1_loss or self.loss == F.mse_loss:
            return self.loss(output, target)
        else:
            output_4d_shape = (output.shape[0], 1, output.shape[1], output.shape[2])
            target_4d_shape = (target.shape[0], 1, target.shape[1], target.shape[2])

            return self.loss(output.reshape(output_4d_shape), target.reshape(target_4d_shape), max_value)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        return {
            ...
            "val_loss": self.calculate_loss(output, target, torch.Tensor([12]).cuda())
        }
